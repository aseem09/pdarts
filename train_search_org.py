...
args.save = '/ceph/aseem-volume/full/search/05_14/logging'
args.tmp_data_dir = '/ceph/aseem-volume/full/search/05_14/data'
...
def main():
    ...
    for sp in range(len(num_to_keep)):
        model_1 = Network(args.init_channels + int(add_width[sp]), CIFAR_CLASSES, args.layers + int(
            add_layers[sp]), criterion, switches_normal=switches_normal_1, switches_reduce=switches_reduce_1, p=float(drop_rate[sp]))
        ...
        model_2 = Network(args.init_channels + int(add_width[sp]), CIFAR_CLASSES, args.layers + int(
            add_layers[sp]), criterion, switches_normal=switches_normal_2, switches_reduce=switches_reduce_2, p=float(drop_rate[sp]))
        ...
        for epoch in range(epochs):
            ...
            if epoch < eps_no_arch:
                model_1.module.p = float(
                    drop_rate[sp]) * (epochs - epoch - 1) / epochs
                model_1.module.update_p()
                model_2.module.p = float(
                    drop_rate[sp]) * (epochs - epoch - 1) / epochs
                model_2.module.update_p()
                train_acc_1, train_acc_2 = train(train_queue, valid_queue, criterion, model_1, network_params_1, optimizer_1,
                                             optimizer_a_1, lr_1, model_2, network_params_2, optimizer_2, optimizer_a_2, lr_2, train_arch=False)
            else:
                model_1.module.p = float(
                ...
                model_2.module.update_p()
                train_acc_1, train_acc_2 = train(train_queue, valid_queue, criterion, model_1, network_params_1, optimizer_1,
                                                 optimizer_a_1, lr_1, model_2, network_params_2, optimizer_2, optimizer_a_2, lr_2, train_arch=True)
            logging.info('Train_acc %f %f', train_acc_1, train_acc_2)
            epoch_duration = time.time() - epoch_start
            logging.info('Epoch time: %ds', epoch_duration)
            # validation
            if epochs - epoch < 5:
                valid_acc_1, valid_acc_2 = infer(
        ...
        normal_prob_1 = F.softmax(
            arch_param_1[0], dim=sm_dim).data.cpu().numpy()
        arch_param_2 = model_2.module.arch_parameters()
        normal_prob_2 = F.softmax(
            arch_param_2[0], dim=sm_dim).data.cpu().numpy()
        for i in range(14):
            idxs = []
            ...
            if sp == len(num_to_keep) - 1:
                ...
                drop = get_min_k_no_zero(
                    normal_prob_1[i, :], idxs, num_to_drop[sp])
            else:
                drop = get_min_k(normal_prob_1[i, :], num_to_drop[sp])
            for idx in drop:
                switches_normal_1[i][idxs[idx]] = False
        for i in range(14):
            idxs = []
            for j in range(len(PRIMITIVES)):
            ...
            if sp == len(num_to_keep) - 1:
                # for the last stage, drop all Zero operations
                drop = get_min_k_no_zero(
                    normal_prob_2[i, :], idxs, num_to_drop[sp])
            else:
                drop = get_min_k(normal_prob_2[i, :], num_to_drop[sp])
            for idx in drop:
                switches_normal_2[i][idxs[idx]] = False
        ...
        for i in range(14):
            ...
            if sp == len(num_to_keep) - 1:
                drop = get_min_k_no_zero(
                    reduce_prob_1[i, :], idxs, num_to_drop[sp])
            else:
                drop = get_min_k(reduce_prob_1[i, :], num_to_drop[sp])
            for idx in drop:
                switches_reduce_1[i][idxs[idx]] = False
        for i in range(14):
            idxs = []
            for j in range(len(PRIMITIVES)):
            ...
            if sp == len(num_to_keep) - 1:
                drop = get_min_k_no_zero(
                    reduce_prob_2[i, :], idxs, num_to_drop[sp])
            else:
                drop = get_min_k(reduce_prob_2[i, :], num_to_drop[sp])
            for idx in drop:
                switches_reduce_2[i][idxs[idx]] = False
        ...
        if sp == len(num_to_keep) - 1:
            ...
            normal_prob_1 = F.softmax(
                arch_param_1[0], dim=sm_dim).data.cpu().numpy()
            reduce_prob_1 = F.softmax(
                arch_param_1[1], dim=sm_dim).data.cpu().numpy()
            ...
            normal_prob_2 = F.softmax(
                arch_param_2[0], dim=sm_dim).data.cpu().numpy()
            reduce_prob_2 = F.softmax(
                arch_param_2[1], dim=sm_dim).data.cpu().numpy()
            ...
            for i in range(3):
                ...
                tbsn = normal_final_1[start:end]
                tbsr = reduce_final_1[start:end]
                edge_n = sorted(range(n), key=lambda x: tbsn[x])
                keep_normal_1.append(edge_n[-1] + start)
                keep_normal_1.append(edge_n[-2] + start)
                edge_r = sorted(range(n), key=lambda x: tbsr[x])
                keep_reduce_1.append(edge_r[-1] + start)
                keep_reduce_1.append(edge_r[-2] + start)
                start = end
                n = n + 1
            ...
            genotype = parse_network(switches_normal_1, switches_reduce_1)
            ...
            for sks in range(0, 9):
                max_sk = 8 - sks
                num_sk = check_sk_number(switches_normal_1)
                if not num_sk > max_sk:
                    continue
                while num_sk > max_sk:
                    normal_prob_1 = delete_min_sk_prob(
                        switches_normal_1, switches_normal_1_copy, normal_prob_1)
                    switches_normal_1 = keep_1_on(
                        switches_normal_1_copy, normal_prob_1)
                    switches_normal_1 = keep_2_branches(
                        switches_normal_1, normal_prob_1)
                    num_sk = check_sk_number(switches_normal_1)
            ...
            for i in range(3):
                ...
                tbsn = normal_final_2[start:end]
                tbsr = reduce_final_2[start:end]
                edge_n = sorted(range(n), key=lambda x: tbsn[x])
                keep_normal_2.append(edge_n[-1] + start)
                keep_normal_2.append(edge_n[-2] + start)
                edge_r = sorted(range(n), key=lambda x: tbsr[x])
                keep_reduce_2.append(edge_r[-1] + start)
                keep_reduce_2.append(edge_r[-2] + start)
                start = end
                n = n + 1
            # set switches according the ranking of arch parameters
            for i in range(14):
            ...
            # translate switches into genotype
            genotype = parse_network(switches_normal_2, switches_reduce_2)
            logging.info(genotype)
            # restrict skipconnect (normal cell only)
            logging.info('Restricting skipconnect...')
            # generating genotypes with different numbers of skip-connect operations
            for sks in range(0, 9):
                max_sk = 8 - sks
                num_sk = check_sk_number(switches_normal_2)
                if not num_sk > max_sk:
                    continue
                while num_sk > max_sk:
                    normal_prob_2 = delete_min_sk_prob(
                        switches_normal_2, switches_normal_2_copy, normal_prob_2)
                    switches_normal_2 = keep_1_on(
                        switches_normal_2_copy, normal_prob_2)
                    switches_normal_2 = keep_2_branches(
                        switches_normal_2, normal_prob_2)
                    num_sk = check_sk_number(switches_normal_2)
                logging.info('Number of skip-connect: %d', max_sk)
                genotype = parse_network(switches_normal_2, switches_reduce_2)
                logging.info(genotype)
...
def train(train_queue, valid_queue, criterion, model_1, network_params_1, optimizer_1, optimizer_a_1, lr_1, model_2, network_params_2, optimizer_2, optimizer_a_2, lr_2, train_arch=True):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            loss_a_1 = criterion(logits_1, target_search)

            logits_2 = model_2(input_search)
            loss_a_2 = criterion(logits_2, target_search)

            loss_a_1.backward()
            loss_a_2.backward()

            nn.utils.clip_grad_norm_(
                model_1.module.arch_parameters(), args.grad_clip)
            nn.utils.clip_grad_norm_(
                model_2.module.arch_parameters(), args.grad_clip)
            optimizer_a_1.step()
            optimizer_a_2.step()
        ...
        if step % args.report_freq == 0:
            logging.info('TRAIN Step: %03d R1: %f R5: %f R1: %f R5: %f',
                         step, top1_1.avg, top5_1.avg, top1_2.avg, top5_2.avg)
...
def infer(valid_queue, model_1, model_2, criterion):
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('valid %03d %f %f %f %f', step, top1_1.avg, top5_1.avg, top1_2.avg, top5_2.avg)
